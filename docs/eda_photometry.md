...
## Координатный CLI и физические единицы

Для массовой обработки по RA/Dec используйте [photometry_cli.py](real_data_workflow.md). GUI предназначен для ручного просмотра. После исправления GUI применяет `EXPTIME` к HST `ELECTRONS/S`, gain к ADU и вычитает оценённый фон. Если калибровка отсутствует, экспорт останавливается, а не принимает коэффициент 1.
